...
O estilo de saída **Padrão** do Claude Code é o prompt do sistema existente, projetado para ajudá-lo a concluir tarefas de engenharia de software com eficiência.
...
Essas alterações se aplicam ao [nível do projeto local](/pt/settings) e são salvas em `.claude/settings.local.json`. Você também pode editar diretamente o campo `outputStyle` em um arquivo de configurações em um nível diferente.
...
| `description`              | Descrição do estilo de saída. Usado apenas na interface do usuário de `/output-style`    | Nenhum                   |
